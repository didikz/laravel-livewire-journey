<nav class="bg-gray-800">
        ...
        <div class="flex items-center justify-between h-16">
            <div class="flex items-center">
                <div class="flex-shrink-0">
                    <img class="w-10" src="/img/logos/workflow-mark-on-dark.svg" alt="logo" />
                </div>
                <div class="hidden md:block">
                    <div class="ml-10 flex items-baseline">
                        <a href="#" class="px-3 py-2 rounded-md text-sm font-medium {{ request()->is('dashboard*') ? 'text-white bg-gray-900' : 'text-gray-300 hover:text-white hover:bg-gray-700' }} focus:outline-none focus:text-white focus:bg-gray-700">Dashboard
                        </a>
                        <a href="{{ route('users.index') }}" class="ml-4 px-3 py-2 rounded-md text-sm font-medium {{ request()->is('users*') ? 'text-white bg-gray-900' : 'text-gray-300 hover:text-white hover:bg-gray-700' }} focus:outline-none focus:text-white focus:bg-gray-700">Users
                        </a>
                    ...
                    <div class="ml-3 relative">
                        ...
                        >
                            <div class="py-1 rounded-md bg-white shadow-xs" role="menu" aria-orientation="vertical" aria-labelledby="user-menu">
                                ...
                                <span href="#" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem">
                                    <form action="{{ route('logout') }}" method="post">
                                        @csrf
                                        <button class="focus:outline-none" type="submit">Sign out</button>
                                    </form>
                                </span>
                            </div>
                        </div>
            ...
            <div class="-mr-2 flex md:hidden">
                <!-- Mobile menu button -->
                <button @click="mobileNavOpen = !mobileNavOpen" class="inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-white hover:bg-gray-700 focus:outline-none focus:bg-gray-700 focus:text-white">
                    <!-- Menu open: "hidden", Menu closed: "block" -->
                    <svg x-state:on="Menu Open" x-state:off="Menu Closed" :class="{ 'hidden': mobileNavOpen, 'block': !mobileNavOpen }" class="block h-6 w-6" stroke="currentColor" fill="none" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
                    </svg>
                    <!-- Menu open: "block", Menu closed: "hidden" -->
                    <svg x-state:on="Menu Open" x-state:off="Menu Closed" :class="{ 'hidden': !mobileNavOpen, 'block': mobileNavOpen }" class="hidden h-6 w-6" stroke="currentColor" fill="none" viewBox="0 0 24 24">
    ...
    >
        <div class="px-2 pt-2 pb-3 sm:px-3">
            <a href="#" class="block px-3 py-2 rounded-md text-base font-medium {{ request()->is('dashboard*') ? 'text-white bg-gray-900' : 'text-gray-300 hover:text-white hover:bg-gray-700' }} focus:outline-none focus:text-white focus:bg-gray-700">Dashboard
            </a>
            <a href="{{ route('users.index') }}" class="mt-1 block px-3 py-2 rounded-md text-base font-medium {{ request()->is('users*') ? 'text-white bg-gray-900' : 'text-gray-300 hover:text-white hover:bg-gray-700' }} focus:outline-none focus:text-white focus:bg-gray-700">Team
            </a>
